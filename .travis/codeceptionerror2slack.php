...
fwrite(STDOUT, "\033[31;1mExist output folder?" . (int) !file_exists($codeceptionOutputFolder) . "\033[0m\n");
fwrite(STDOUT, "\033[31;1mDo output folder has images?" . (int) !(new \FilesystemIterator($codeceptionOutputFolder))->valid() . "\033[0m\n");
...
if ($handler = opendir($codeceptionOutputFolder))
{
	while (false !== ($errorSnapshot = readdir($handler)))
	{
		...
		fwrite(STDOUT, "\033[31;1mError Snapshot: " . $errorSnapshot . "\033[0m\n");
		...
		$command = 'curl -F file=@' . $codeceptionOutputFolder . '/' . $errorSnapshot . ' -F '
			...
			. 'initial_comment="error found by travis in redCORE on build:' . getenv('TRAVIS_BUILD_NUMBER') . '" -F'
			. 'token=' . $slackToken . ' '
